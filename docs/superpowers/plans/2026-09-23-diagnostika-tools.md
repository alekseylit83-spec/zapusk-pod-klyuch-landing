...
- Пустой прогон необязательных полей (текстовых, числовых, мультивыбора,
  концепции, параметров помещения — «Далее» на них никогда не блокируется
  пустым значением) — итоговый текст не должен содержать «undefined» или
  пустые строки вида «Формат: ». Шаги-выборы (шкала, одиночный выбор,
  да/нет) под это не подпадают — они продвигают визард сразу по нажатию
  варианта, оставить их пустыми через обычную навигацию нельзя.
...
2. Ответить «Не под контролем» на два-три пункта раздела А (например,
   ...
   **первый** из отвеченных красных пунктов по порядку списка (пункт 1,
   ...
   тем же.
3. Ответить на все 7 пунктов раздела А любыми вариантами, оставить план-факт
   по выручке и все поля снимка объекта пустыми — на финальном экране должны
   быть строки подсчёта и «Главный риск», но не должно быть строки
   «План-факт по выручке» и не должно быть строк снимка объекта (Review
   Focus, пункт 1: пустые необязательные поля не должны печататься как
   «undefined»).
